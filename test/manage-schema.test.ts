...
describe("management schema",()=>{
  test("fresh control DB is v2 with the exact management tables",()=>{const db=dbFixture();expect(db.query("SELECT version FROM control_schema_meta WHERE id=1").get()).toEqual({version:2});expect((db.query("SELECT name FROM sqlite_master WHERE type='table' AND name LIKE 'mgmt_%' ORDER BY name").all() as {name:string}[]).map(x=>x.name)).toEqual(tables);db.close();});
  test("v1 migrates without touching control works",()=>{const db=dbFixture();work(db,"keep");for(const name of [...tables].reverse())db.exec(`DROP TABLE ${name}`);db.query("UPDATE control_schema_meta SET version=1 WHERE id=1").run();ensureControlSchema(db);expect(db.query("SELECT work_id,title FROM control_works").all()).toEqual([{work_id:"keep",title:"keep"}]);expect(db.query("SELECT version FROM control_schema_meta WHERE id=1").get()).toEqual({version:2});db.close();});
