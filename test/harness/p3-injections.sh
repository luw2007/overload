...
NOTIFIER="${NOTIFIER:-$ROOT/src/notify/notifier.ts}"
...
while [[ $# -gt 0 ]]; do case "$1" in
  ...
  --notifier) NOTIFIER="$2"; shift 2;; -h|--help) echo "usage: $0 [--only 1,2,3] [--strict]"; exit 0;; *) echo "unknown arg: $1" >&2; exit 2;; esac; done
...
runnote(){ env HOME="$W" bun "$NOTIFIER" --once --sink "file:$W/sink" >/dev/null; }
scenario1(){
  local m; m="$(missing "$PULL" "$INGEST" "$NOTIFIER")"; [[ -z "$m" ]] || { record 1 SKIP "missing entry: $m"; return; }; new; make_cmds
  mkdir -p "$P3_REMOTE/pi-p3-fake"; envline > "$P3_REMOTE/pi-p3-fake/active-pi-p3-fake-1.ndjson"
  t0="$(bun -e 'console.log(Date.now())')"; runpull >/dev/null; runingest; runnote; t1="$(bun -e 'console.log(Date.now())')"
  [[ -s "$W/sink" ]] || { record 1 FAIL "notification sink is empty"; return; }
  echo "write→sent latency: $((t1-t0))ms"; record 1 PASS "local fake devbox pull→ingest→file notification";
}
scenario2(){
