...
#
# Usage:
#   test/harness/p2-injections.sh [--only 1,2,3,4] [--keep] [--strict]
...
# ── SCENARIO 1 — ask unanswered → Q1 ────────────────────────────────────────
scenario_1() {
  local missing
  missing="$(entry_missing "$CLASSIFIER" "$INGEST" "$CLI")"
  ...
  if [[ "$ok" -ne 1 ]] || ! grep -q "$uid" <<<"$out"; then
    ...
    record 1 PASS "q1 lists pending ask"
  fi
  cleanup_work
}
...
# ── run ──────────────────────────────────────────────────────────────────────
echo "## p2-injections: four-injection acceptance (freeze table)"
echo "## entries: ingest=$INGEST recon=$RECON cli=$CLI classifier=$CLASSIFIER"
echo
