...
import { detectRuntimeFromPath, parseClaudeSession, parseOmpSession, parsePiSession, sessionDirs, type SessionRecord, type PendingCall } from "./readers/types";
...
function strongWork(db:Database,ledger:Database|null,sid:string):string|null{
  ...
  for(const ref of refs){
    const handoff=/^(?:mgmt:handoff:)(.+)$/.exec(ref);if(handoff){const h=one<any>(db,"SELECT work_id FROM mgmt_handoffs WHERE handoff_id=?",handoff[1]);if(h)return canonicalWorkId(db,h.work_id);}
    const parent=one<any>(db,"SELECT work_id FROM mgmt_session_binding WHERE stable_id=?",ref);if(parent)return canonicalWorkId(db,parent.work_id);
  }
  return null;
}
...
export async function scanOnce(db:Database,ledger:Database|null,cfg:ManageConfig,opts:{now?:number;fsFor?:(h:SourceHost)=>SourceFs}={}):Promise<ScanReport>{
  ...
  for(const h of cfg.hosts){const fs=opts.fsFor?.(h)??(h.kind==="local"?localSourceFs(h):sshSourceFs(h));try{
    ...
    for(const file of files.values()){
      ...
      const newline=range.bytes.lastIndexOf(10), completeBytes=newline+1;
      if(completeBytes===0){report.skipped++;continue;}
      const chunk=new TextDecoder().decode(range.bytes.subarray(0,completeBytes));const text=cursor.header?`${cursor.header}\n${chunk}`:chunk;
      const record=parser(runtime,text,{lineOffset:cursor.header?(cursor.line||0)-1:0,pendingCalls:cursor.pendingCalls??[]});if(!record){updateCursor(db,sourceKey,{...cursor,byte:range.nextByte,generation:range.generation},"parse_error",now);report.skipped++;continue;}
      ...
      refreshWorkHints(db,canonicalWorkId(db,workId),now);
      const firstLine=text.split(/\r?\n/).find(Boolean)??cursor.header;updateCursor(db,sourceKey,{byte:cursor.byte+completeBytes,generation:range.generation,header:firstLine,line:record.lastLineNo,pendingCalls:record.pendingCalls??[]},range.eof?"eof":"partial",now);
    }
...
export function showWork(db:Database,workId:string):WorkDetail|null{
  ensureMgmtSchema(db);
  const profile=one<any>(db,"SELECT w.*,p.* FROM control_works w JOIN mgmt_work_profile p USING(work_id) WHERE w.work_id=?",workId);
  if(!profile)return null;
  const canonical=canonicalWorkId(db,workId), scope=canonical===workId?workScope(db,workId):[workId], marks=scope.map(()=>"?").join(",");
  return {...profile,canonical_work_id:canonical,scope_work_ids:scope,
    aliases:all(db,"SELECT a.*,w.title FROM mgmt_work_alias a JOIN control_works w ON w.work_id=a.alias_work_id WHERE a.canonical_work_id=?",canonical),
    hints:all(db,"SELECT h.*,w.title FROM mgmt_work_hints h JOIN control_works w ON w.work_id=h.other_work_id WHERE h.work_id=?",workId),
    executions:all(db,`SELECT * FROM mgmt_executions WHERE work_id IN (${marks}) ORDER BY started_at`,...scope),
    inputs:all(db,`SELECT * FROM mgmt_inputs WHERE work_id IN (${marks}) ORDER BY version DESC`,...scope),
    artifacts:all(db,`SELECT a.*,v.version_id,v.content_sha256,v.snapshot_state,v.sensitivity,v.shareable,v.observed_at,v.producer,v.evidence_at,v.stale_capture FROM mgmt_artifacts a LEFT JOIN mgmt_artifact_versions v ON v.artifact_id=a.artifact_id WHERE a.work_id IN (${marks}) ORDER BY a.display_path,v.observed_at DESC`,...scope),
    links:all(db,`SELECT * FROM mgmt_links WHERE work_id IN (${marks}) ORDER BY observed_at DESC`,...scope),
    handoffs:all(db,`SELECT * FROM mgmt_handoffs WHERE work_id IN (${marks}) ORDER BY rowid DESC`,...scope),
    attention:all(db,`SELECT * FROM control_attention WHERE work_id IN (${marks}) AND state!='resolved'`,...scope)};
}
export function setTracking(db:Database,workId:string,on:boolean):void{ensureMgmtSchema(db);if(canonicalWorkId(db,workId)!==workId)throw new ControlError("conflict","aliased_work");const r=db.query("UPDATE mgmt_work_profile SET track_state=?,archived_at=CASE WHEN ? THEN NULL ELSE archived_at END,updated_at=? WHERE work_id=?").run(on?"tracking":"paused",on?1:0,Date.now(),workId);if(r.changes!==1)throw new ControlError("not_found","work not found");}
