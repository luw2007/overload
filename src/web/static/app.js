(() => {
  ...
  async function resolveItem(id,option) {const item=findAttention(id);if(option==='narrow')return openNarrow(item);if(option==='defer'){const result=await post(`/api/attention/${encodeURIComponent(id)}/defer`,{expected_revision:item.revision,defer_until:Date.now()+3600000});receipts.set(id,`<div class="row receipt">defer · you · ${stamp(Date.now())} · until ${stamp(result.defer_until||Date.now()+3600000)}</div>`);render();setTimeout(()=>{receipts.delete(id);refresh();},3000);return;}const result=await post(item.approval_id?`/api/orchestrator/answer/${encodeURIComponent(item.approval_id)}`:`/api/attention/${encodeURIComponent(id)}/resolve`,item.approval_id?{answer:option,consumer_owner:item.consumer_owner,expected_revision:item.revision}:{expected_revision:item.revision,selected_option:option});receipts.set(id,`<div class="row receipt">${e(option)} · you · ${stamp(Date.now())} · effect ${result.effect_state==='succeeded'?'verified':'pending'}</div>`);render();setTimeout(()=>{receipts.delete(id);refresh();},3000);}
