...
import { addTask, bindTaskContract, getTask, listTasks, openStore, transition, type TaskState, STATES } from "./store";
...
function usage():never{throw new Error('usage: overload orch add "<title>" --repo <path> --work-id <work_id> [--base-ref <ref>] | ls [--state <s>] | show <task_id> | answer <approval_id> <option> | advance <task_id> <event> | abandon <task_id> | reopen <task_id> | gc [--older-than <duration>] [--apply]');}
...
export async function runCli(argv=Bun.argv.slice(2),out=(s:string)=>console.log(s)):Promise<void>{if(argv[0]==="orch")argv=argv.slice(1);const [command,...rest]=argv;if(command==="answer"){if(rest.length!==2)usage();const answers=openAnswersDb(process.env.OVERLOAD_ANSWERS_PATH);try{answers.run("INSERT INTO answers(approval_id,answer,actor,at) VALUES(?,?,?,?)",[rest[0]!,rest[1]!,"cli",Date.now()]);out(JSON.stringify({approval_id:rest[0],answer:rest[1],actor:"cli"}));}finally{answers.close();}return;}const db=openStore();try{
 if(command==="add"){const title=rest[0],repoArg=option(rest,"--repo"),workId=option(rest,"--work-id");if(!title||!repoArg||!workId)usage();const repo=resolve(repoArg);const ref=option(rest,"--base-ref")??"HEAD";const resolved=await defaultCommandExecutor("git",["-C",repo,"rev-parse","--verify","--end-of-options",`${ref}^{commit}`]);if(!resolved.ok)throw new Error(`base ref not resolvable: ${ref}`);const answers=openAnswersDb(process.env.OVERLOAD_ANSWERS_PATH);try{const work=getWork(answers,workId);if(!work||!work.contract||work.state!=="active")throw new Error("active contracted work required");const task=addTask(db,title,repo,resolved.stdout.trim());out(JSON.stringify(bindTaskContract(db,task.task_id,work)));}finally{answers.close();}}
