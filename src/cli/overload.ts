...
type RequestRow = { request_uid: string; stable_id: string; kind: string; created_at: number; detail: string | null; failed: number; binding: string | null };
...
function usage(): never { console.error("usage: overload sessions | show <stable_id> | q1 | q2 | zombie | health"); process.exit(2); }
...
function showSession(db: Database, stableId: string): void {
  ...
  console.log("\nPending requests:");
  const requests = db.query(`SELECT request_uid, stable_id, kind, created_at, detail, 0 failed, NULL binding FROM requests WHERE stable_id=? AND state='pending' ORDER BY created_at, request_uid`).all(stableId) as RequestRow[];
...
function q1(db: Database): void {
  const rows = db.query(`SELECT r.request_uid, r.stable_id, r.kind, r.created_at, r.detail,
    ...
    FROM requests r WHERE r.state='pending' ORDER BY failed DESC, r.created_at, r.request_uid`).all() as RequestRow[];
  if (!rows.length) { console.log("Q1: no pending requests."); return; }
  console.log("Q1 pending requests:");
  for (const row of rows) console.log(`${row.failed ? "[DELIVERY FAILED] " : ""}${row.request_uid}\t${row.kind}\t${time(row.created_at)}\tjump=${row.binding ?? "-"}${detail(row.detail)}`);
}
...
function main(): void {
  ...
  const noArg = new Set(["sessions", "q1", "q2", "zombie", "health"]);
  if (extra.length || !command || (!noArg.has(command) && command !== "show") || (command === "show" && !argument) || (noArg.has(command) && argument)) usage();
  ...
  try { if (command === "sessions") listSessions(db); else if (command === "show") showSession(db, argument!); else if (command === "q1") q1(db); else if (command === "q2") q2(db); else if (command === "zombie") zombie(db); else health(db); } finally { db.close(); }
}
