...
import type {ChannelAdapter,ChannelEvent,ChannelMessage,DeliveryReceipt} from './types';

export type FeishuChannelConfig={appId:string;appSecret:string;instanceId:string;createChannel?:FeishuChannelFactory};

type FeishuSdkMessage=Pick<NormalizedMessage,'messageId'|'chatId'|'chatType'|'senderId'|'content'|'threadId'|'rootId'|'createTime'|'mentionedBot'|'raw'>;
type FeishuSdkAction=Pick<CardActionEvent,'messageId'|'chatId'|'operator'|'action'|'raw'>;
type FeishuSdkChannel={
 on(name:'message',handler:(message:FeishuSdkMessage)=>void|Promise<void>):()=>void;
 on(name:'cardAction',handler:(action:FeishuSdkAction)=>void|Promise<void>):()=>void;
 on(name:'error',handler:(error:unknown)=>void):()=>void;
 connect():Promise<void>;
 disconnect():Promise<void>;
 send(to:string,input:{markdown:string}|{card:object},options?:{replyTo?:string;replyInThread?:boolean}):Promise<{messageId:string}>;
 updateCard(messageId:string,card:object):Promise<void>;
 editMessage(messageId:string,text:string):Promise<void>;
};
type FeishuChannelFactory=(options:Parameters<typeof createLarkChannel>[0])=>FeishuSdkChannel;

function record(value:unknown):Record<string,unknown>{if(!value||typeof value!=='object'||Array.isArray(value))throw new Error('invalid_feishu_object');return value as Record<string,unknown>;}
function requiredString(value:unknown,name:string):string{if(typeof value!=='string'||!value)throw new Error('invalid_feishu_'+name);return value;}
function rawField(raw:unknown,name:string):unknown{const value=record(raw);if(value[name]!==undefined)return value[name];const header=value.header;return header&&typeof header==='object'&&!Array.isArray(header)?(header as Record<string,unknown>)[name]:undefined;}
function rawIdentity(raw:unknown,fallbackEventId:string):{eventId:string;tenantId:string}{return {eventId:requiredString(rawField(raw,'event_id')??fallbackEventId,'event_id'),tenantId:requiredString(rawField(raw,'tenant_key'),'tenant_key')};}
function receivedAt(value:number):number{if(!Number.isFinite(value)||value<=0)return Date.now();return value<1_000_000_000_000?value*1000:value;}
function actionValue(value:unknown):Record<string,unknown>{if(typeof value==='string'){try{return record(JSON.parse(value));}catch{throw new Error('invalid_feishu_action_value');}}return record(value);}
function errorCode(error:unknown):string{if(!error||typeof error!=='object'||Array.isArray(error))return '';const code=(error as Record<string,unknown>).code;return typeof code==='string'?code:'';}
...
export class FeishuChannel implements ChannelAdapter{
 ...
 constructor(readonly config:FeishuChannelConfig){
  ...
  const factory=config.createChannel??(createLarkChannel as unknown as FeishuChannelFactory);
 ...
 async start(accept:(event:ChannelEvent)=>Promise<void>):Promise<void>{
  ...
  this.unsubscribe=()=>{removeMessage();removeAction();};
  try{await this.channel.connect();this.started=true;}catch(error){this.unsubscribe();this.unsubscribe=()=>{};this.accept=null;throw error;}
 }
 async stop():Promise<void>{this.unsubscribe();this.unsubscribe=()=>{};this.accept=null;if(this.started){this.started=false;await this.channel.disconnect();}}
 ...
 private async handleAction(action:FeishuSdkAction):Promise<void>{
  ...
  if(typeof revision!=='number'||!Number.isSafeInteger(revision))throw new Error('invalid_feishu_revision');
  const answer=requiredString(value.answer,'answer');
  const identity=rawIdentity(action.raw,`card:${action.messageId}:${action.operator.openId}:${JSON.stringify(action.action.value)}`);
  const threadId=typeof value.threadId==='string'&&value.threadId?value.threadId:undefined;
  await this.accept({kind:'decision',eventId:identity.eventId,identity:{instanceId:this.instanceId,tenantId:identity.tenantId,userId:requiredString(action.operator.openId,'operator')},address:{instanceId:this.instanceId,tenantId:identity.tenantId,chatId:requiredString(action.chatId,'chat'),...(threadId?{threadId}: {})},messageId:requiredString(action.messageId,'message'),itemId,revision,answer,receivedAt:Date.now()});
 }
