...
export interface ChannelAdapter{readonly kind:string;readonly instanceId:string;readonly capabilities:{update:boolean;actions:boolean};start(accept:(event:ChannelEvent)=>Promise<void>):Promise<void>;stop():Promise<void>;send(message:ChannelMessage):Promise<DeliveryReceipt>;addReaction?(messageId:string,emojiType:string):Promise<string>;removeReaction?(messageId:string,reactionId:string):Promise<void>}
