...
class FakeLarkChannel{
 ...
 on(name:string,handler:Handler){this.handlers.set(name,handler);return ()=>{if(this.handlers.get(name)===handler)this.handlers.delete(name);};}
 async connect(){await this.connectPromise;this.connected=true;}
 async disconnect(){this.connected=false;}
 async send(to:string,input:unknown,options?:unknown){this.sent.push({to,input,options});return {messageId:'sent-1'};}
 async updateCard(messageId:string,card:object){this.updates.push({messageId,card});}
 async editMessage(messageId:string,text:string){this.edits.push({messageId,text});}
 async emit(name:string,value:unknown){await this.handlers.get(name)?.(value);}
}

let fake:FakeLarkChannel;
let factoryOptions:Record<string,unknown>;
const createChannel:NonNullable<FeishuChannelConfig['createChannel']> = options=>{
 factoryOptions=options as unknown as Record<string,unknown>;
 fake=new FakeLarkChannel();
 return fake as unknown as ReturnType<NonNullable<FeishuChannelConfig['createChannel']>>;
};
const config={appId:'cli_0123456789abcdef',appSecret:'secret',instanceId:'feishu-main',createChannel};
function channel(){return new FeishuChannel(config);}
function raw(eventId='event-1',tenantId='tenant-1'){return {event_id:eventId,tenant_key:tenantId};}

 test('uses the official websocket transport and does not report start before connect',async()=>{
  const instance=channel();
  let release!:()=>void;
  fake.connectPromise=new Promise<void>(resolve=>{release=resolve;});
  const starting=instance.start(async()=>{});
  await Promise.resolve();
  expect(factoryOptions.transport).toBe('websocket');
  expect(factoryOptions.policy).toEqual({requireMention:true,dmMode:'open'});
...
test('normalizes authenticated direct and mentioned group messages with thread identity',async()=>{
 ...
 await fake.emit('message',{messageId:'ignored',chatId:'chat',chatType:'group',senderId:'user',content:'ignored',mentionedBot:false,createTime:0,raw:raw('ignored')});
...
test('classifies official SDK delivery errors without pretending success',async()=>{
 const instance=channel();await instance.start(async()=>{});
 fake.send=async()=>{throw Object.assign(new Error('rate limited'),{code:'rate_limited'});};
 expect(await instance.send({deliveryId:'d',address:{instanceId:'feishu-main',tenantId:'tenant',chatId:'chat'},text:'hello'})).toEqual({state:'retryable',reason:'rate_limited'});
 fake.send=async()=>{throw Object.assign(new Error('denied'),{code:'permission_denied'});};
 expect(await instance.send({deliveryId:'d2',address:{instanceId:'feishu-main',tenantId:'tenant',chatId:'chat'},text:'hello'})).toEqual({state:'failed',reason:'permission_denied'});
 fake.send=async()=>{throw Object.assign(new Error('timeout'),{code:'send_timeout'});};
 expect(await instance.send({deliveryId:'d3',address:{instanceId:'feishu-main',tenantId:'tenant',chatId:'chat'},text:'hello'})).toEqual({state:'unknown',reason:'send_timeout'});
 await instance.stop();
});
