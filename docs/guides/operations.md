...
The primary operating surface is Now / Inbox / Done Attention. The CLI `now`, `inbox`, `done` commands and the dashboard's Decide page read and write the same Attention data. Q1–Q5 are internal classifications and implementation details, reachable only from the Agents diagnosis pages — they are not a surface the operator works from day to day.
...
Rule proposals and automatic decisions are an advanced, optional capability, off by default. The core human decision flow below — answering a gated card, and editing its narrow contract — is available without enabling the bot.
...
The commands below are low-level diagnostic and maintenance tools. The primary user entry remains Now / Inbox / Done Attention; these exist for inspecting internal classifications (Q1–Q5), hung turns, and incidents.
