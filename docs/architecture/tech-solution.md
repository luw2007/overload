...
研究基础：`docs/research/overload-20260813-*.md`。对抗评审：`<provider>/<model>` 七轮（R1-R6 OBJECT 共 11 blocker + 18 major + 12 minor 全部消解，R7 ALIGN）。消解映射见 §9。取代 `overload-20260813-ledger-design.md`（其 §2/§5 队列与 schema 骨架被本方案 §2.3/§2.4 吸收修订）。
...
- **实施状态**：已实现（src/ingest/schema.sql `source_generations` 表 + cmux.ts workstream 解析）。契约经 sol R5/R6 对齐，照此执行。
