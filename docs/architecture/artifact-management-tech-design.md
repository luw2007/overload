...
> 实现状态：已实现（src/manage/ + src/cli/mgmt.ts）。本文为设计记录。
...
- **方案建议**：本文提出的设计，设计时未实现，现已落地（src/manage/ + src/cli/mgmt.ts）。
...
| B. 新建独立管理服务 + 独立 DB（含 v1 的 `mgmt_tasks`） | 边界清晰 | **已否决**：与 `control_works`/`orchestrator.tasks` 形成三套任务状态且无同步权威；违反 Prompt"不并行新增语义相同的任务系统"（`docs/history/plans/artifact-management-prompt.md:141`） |
...
"目标等级"是本方案**建议达到**的接入级别。**当前 mgmt 实现：设计时未实现，现已落地**——`src/manage/`、session jsonl 读取器与 `mgmt_*` schema 均已存在。"原材料"列是**可读取的素材**，不是已实现的采集。
...
| prime-agent | extension（路径未验证，`docs/guides/operations.md:47-49`） | `<host>:prime:<uuid>` | 未验证 | 未验证 | 不支持 | 事件层（不入首批） |
...
{ "handoff_id":"h_5f3c…", "work_id":"w_9a1e…", "manifest_id":"m_77…", "created_at":1789137600000,
  ...
  "artifacts":[{"artifact_id":"a_1c…","kind":"file","path":"docs/architecture/…-tech-design.md","version_id":"v_88…",
...
POST /works/w_9a1e/handoffs {"target_agent":"omp","isolate":true,
     "override_reason":"Claude 会话无 ledger 证据，已确认终端已关","override_actor":"<actor>"}
200 {"handoff_id":"h_7a21","state":"ready_to_launch","isolate":true,
     "preconditions":{"termination":"unknown","coverage":"file_only","confirmed_by":"<actor>"},
...
- §6.1 的等级列为 **目标等级**；实际接入状态以 `src/manage/` 现有采集为准。
- 设计撰写时未实现任何代码、未运行任何迁移；相应代码、迁移与验收测试现已落地，§13 描述以现有测试为准。
