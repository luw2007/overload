...
`docs/guides/integrations.md:7` 同步加作用域限定。
...
| 文档漂移 | `docs/guides/operations.md:86`、`launchd/README.md:14`（「emits no macOS notifications」已不实）、`docs/guides/integrations.md:7,22-25`（hook 已删的孤儿段）、`README.md:73` 起代码围栏未闭合（全文 ```` ``` ```` 计数为 11，奇数）、`AGENTS.md:29` 加 D2 限定、`docs/history/contracts/*` 与旧 plan 顶部加「历史文档」标注 | +20/−10 |
...
- 新建：`src/orchestrator/schema.sql`、`store.ts`（claim 事务、租约、转移 + `task_events`）、`spool.ts`（§4.1 轮转 writer）、`orchestrator.ts`（5s tick + reconcile）、`cli.ts`（`add|ls|show|advance|abandon|reopen`）、`launchd/app.overload.orchestrator.plist`
...
| `docs/history/plans/*` 历史文档 | 只加「历史文档」标注，保留设计推理 |
