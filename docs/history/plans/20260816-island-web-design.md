...
- launchd：新增 `app.overload.web.plist`，`KeepAlive`，同 ingest/notifier 模式，命令 `bun src/web/server.ts`。
...
- 构建/部署：`swift build -c release` 产出单一可执行文件；新增 `app.overload.island.plist`（`RunAtLoad`，GUI domain，路径指向编译产物）。
